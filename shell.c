...
/**
 * Function prototype for SIGINT signal handler.
 *      NOTE: standard for function prototypes is to not use argument names so that name can be
 *            chosen upon implementation of the function.
 */
...
int main(void) {
    ...
    // Makes SIGINT not interrupt the WCU shell.
...
/**
 * This function will terminate the child process when the SIGINT signal is sent to the shell.
 * If no child processes exist, the signal will be ignored.
 *
 * @params sigNum The integer representation of the signal sent to the process.
 */
...
int dup_wrapper(int oldfd) {
    ...
    int dupRetVal; //TODO Should this be -1 initially? Is that safer?
